...
EdScoreDS := DATASET('~UGA::Main::Hacks::EducationSummary',EducationScoreRec,THOR);
...
MyOutRec := RECORD
    EdScoreDS.State;
    EdScoreDS.avestratio;
    DECIMAL5_4 NormalizedEducation;
END;

MyOutRec CatThem(EdScoreDS L ) := TRANSFORM
    SELF.NormalizedEducation := -1 * (L.avestratio - AVE(EdScoreDs, avestratio) )/ SQRT(VARIANCE(EdScoreDs, avestratio) );
    SELF := L;
END;

EXPORT NormEduScores := PROJECT(EdScoreDS,
                   CatThem(LEFT));

//OUTPUT(NormEduScores,,'~UGA::Main::Hacks::NormalizedEducation',NAMED('NormalizedEducationScores'),OVERWRITE);



//OUTPUT(AddPubScore,,'~UGA::Main::Hacks::EducationScores',NAMED('Scores'),OVERWRITE);
